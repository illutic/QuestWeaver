...
class RecentGamesLocalDataSource(
    ...
    override suspend fun getRecentGames(): List<GameDto> =
        withContext(ioDispatcher) {
            recentGamesDataStore.data.first()
        }

    override suspend fun getGame(gameId: String): GameDto? = getRecentGames().firstOrNull { it.gameId == gameId }

    override suspend fun saveGame(gameDto: GameDto): Unit =
        withContext(ioDispatcher) {
    ...
    override suspend fun removeGame(gameId: String): Unit =
        withContext(ioDispatcher) {
            recentGamesDataStore.updateData { recentGames ->
                recentGames.toMutableList().apply {
                    removeIf { recentGame -> recentGame.gameId == gameId }
                }
            }
        }
}
